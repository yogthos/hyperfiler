...
class HyperFiler {
    ...
    constructor(options) {
        ...
        hyperfiler_options_1.HyperFilerPrebuiltOptions.DEFAULT, 
        // Adding prebuilt HyperFiler options.
        options.sanitize
            ? hyperfiler_options_1.HyperFilerPrebuiltOptions.SANITIZE
            : {}, options.compatibility
            ? hyperfiler_options_1.HyperFilerPrebuiltOptions.COMPATIBILITY
            : {}, options.simpleMinification
            ? hyperfiler_options_1.HyperFilerPrebuiltOptions.SIMPLE_MINIFICATION
            : {}, options.advancedMinification
            ? hyperfiler_options_1.HyperFilerPrebuiltOptions.ADVANCED_MINIFICATION
            : {}, options.hyperMinification
            ? hyperfiler_options_1.HyperFilerPrebuiltOptions.HYPER_MINIFICATION
            : {}, 
        ...
        this.baseUrl = utilities.getBaseUrl(this.absoluteUrl, this.protocol);
        ...
        const isHeadersEmpty = Object.keys(headers).length === 0;
     ...
     */
    getProtocol(url) {
        const protocolRegex = /^(([a-zA-Z0-9]*?:[/]{2})|(data:))/i;
    ...
    resolveAbsoluteUrl(url) {
        ...
        // will be used.
        const protocol = this.getProtocol(url) === undefined
            ? 'file:'
     ...
     */
    createCacheResource(url, type) {
        // Getting the absolute URL and protocol of the resource.
        const absoluteUrl = this.resolveAbsoluteUrl(url);
        const protocol = this.getProtocol(url);
        // Creating the new resource.
        const resource = new resource_1.Resource(url, absoluteUrl, protocol, type, this.transportOptions);
     ...
     */
    fetchCachedResourceByType(type) {
        return __awaiter(this, void 0, void 0, function* () {
            // Getting a subcache of only the provided resource type.
            const subCache = this.createSubCache(type);
    ...
    cacheCss() {
        ...
        for (const linkStyleElement of linkStyleElements) {
            const url = linkStyleElement.getAttribute('href');
            const type = resource_1.ResourceType.CSS;
    ...
    cacheScripts() {
        ...
        for (const scriptElement of scriptElements) {
            ...
            const type = resource_1.ResourceType.SCRIPT;
            this.createCacheResource(url, type);
        }
    }
    /**
    ...
    cacheHtmlImages() {
        ...
        for (const imageSrcElement of imageSrcElements) {
            const url = imageSrcElement.getAttribute('src');
            this.createCacheResource(url, type);
        }
        // Caching all image URLs in the `srcset` attribute of `<img>` and
        // `<picture>/<source>` tags.
        const imageSrcsetElements = [
        ...
        for (const imageSrcsetElement of imageSrcsetElements) {
            const srcset = imageSrcsetElement.getAttribute('srcset');
                    ...
                    if (imageProperties.includes(propertyName)) {
                        cssTree.walk(declarationNode, {
                            ...
                            enter: (urlNode) => {
                                ...
                                const url = urlNode.value.value.replace(/("|')/gm, '');
                                imageUrls.push(url);
                            },
                        });
                    }
    ...
    cacheAudio() {
        // Selecting the resource type that will be added to the resource cache.
        const type = resource_1.ResourceType.AUDIO;
    ...
    cacheVideos() {
        // Selecting the resource type that will be added to the resource cache.
        const type = resource_1.ResourceType.VIDEO;
    ...
    cacheFonts() {
        // Selecting the resource type that will be added to the resource cache.
        const type = resource_1.ResourceType.FONT;
    ...
    cacheCursors() {
        // Selecting the resource type that will be added to the resource cache.
        const type = resource_1.ResourceType.CURSOR;
    ...
    cacheOtherResources() {
        // Selecting the resource type that will be added to the resource cache.
        const type = resource_1.ResourceType.OTHER;
        ...
        for (const embedElement of embedElements) {
            const url = embedElement.getAttribute('src');
            this.createCacheResource(url, type);
        }
        ...
        for (const objectElement of objectElements) {
            const url = objectElement.getAttribute('data');
            this.createCacheResource(url, type);
        }
    }
    /**
     ...
     */
    rebaseCssUrlFunctions(css, baseUrl) {
        ...
        const rebasedCss = cssTree.generate(cssAst);
        return rebasedCss;
    }
    /**
     ...
     */
    importCss(css, baseUrl) {
        var _a;
        return __awaiter(this, void 0, void 0, function* () {
            // Getting an array of all `@import` statements in the CSS.
            const importRegex = /[@]import\s.*?([;]|\n)/gmis;
            const cssImports = (_a = css.match(importRegex)) !== null && _a !== void 0 ? _a : [];
            ...
            for (const cssImport of cssImports) {
                ...
                const importUrl = cssImport
                    .replace(/([@]import\s*?|url\(|\)|["';])/gmis, '')
                    .trim();
                const importProtocol = this.getProtocol(importUrl);
                const importAbsoluteUrl = utilities.resolveAbsoluteUrl(importUrl, importProtocol, baseUrl);
                ...
                const resource = new resource_1.Resource(importUrl, importAbsoluteUrl, importProtocol, resource_1.ResourceType.CSS, this.transportOptions);
                // Fetching the CSS resource.
                yield resource.fetch();
                ...
                if (status === true) {
                    ...
                    const importCss = bytes.toString();
                    const rebasedCss = this.rebaseCssUrlFunctions(importCss, baseUrl);
                    // Adding the imported and rebased CSS to the base CSS.
                    css = css.split(cssImport).join(rebasedCss);
                    if (css.includes('@import')) {
                        const importBaseUrl = utilities.getBaseUrl(importAbsoluteUrl, importProtocol);
                        css = yield this.importCss(css, importBaseUrl);
                    }
                }
            }
            return css;
    ...
    importAllCss() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // fetched.
            yield this.fetchCachedResourceByType(resource_1.ResourceType.CSS);
            // Creating a CSS resource subcache.
            const styleResources = this.createSubCache(resource_1.ResourceType.CSS);
    ...
    inlineScripts() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // been fetched.
            yield this.fetchCachedResourceByType(resource_1.ResourceType.SCRIPT);
    ...
    inlineHtmlImages() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const imageSrcsetElement of imageSrcsetElements) {
                ...
                // attribute.
                let srcset = imageSrcsetElement.getAttribute('srcset');
                const urls = utilities.parseSrcsetUrls(srcset);
                for (const url of urls) {
    ...
    inlineAudio() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // been fetched.
            yield this.fetchCachedResourceByType(resource_1.ResourceType.AUDIO);
    ...
    inlineVideos() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // been fetched.
            yield this.fetchCachedResourceByType(resource_1.ResourceType.VIDEO);
     ...
     */
    inlineOtherResources() {
        return __awaiter(this, void 0, void 0, function* () {
            // Fetching all other resources in the resource cache that have not yet
            // been fetched.
            yield this.fetchCachedResourceByType(resource_1.ResourceType.OTHER);
    ...
    inlineCssResources() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // fetched.
            yield this.fetchAllCachedResources();
            ...
            for (const styleElement of styleElements) {
                ...
                const resourceInlinedCss = cssTree.generate(cssAst);
     ...
     */
    runPlugins(hyperFilerPlugins) {
        return __awaiter(this, void 0, void 0, function* () {
            for (const hyperFilerPlugin of hyperFilerPlugins) {
                yield hyperFilerPlugin(this);
            }
        });
    }
    /**
    ...
    run() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // resource.
            this.createCacheResource(this.url, resource_1.ResourceType.HTML);
            ...
            logger_1.logger.log('info', 'Running the `beforeInitialFetch` plugins.');
            yield this.runPlugins(this.options.plugins.beforeInitialFetch);
            // Fetching the HTML page resource.
            logger_1.logger.log('info', 'Fetching the entry URL page.');
            yield this.entryResource.fetch();
            // Setting the HTML property to the fetched page.
            logger_1.logger.log('info', 'Building the initial document.');
            ...
            const jsdomOptions = this.protocol === 'file:'
                ? {}
                : { url: this.absoluteUrl };
            this.document = new jsdom_1.JSDOM(this.html, jsdomOptions)
                .window
                .document;
            // Running the `afterInitialFetch` and `beforePreCssInlineRemoval` plugins.
            logger_1.logger.log('info', 'Running the `afterInitialFetch` plugins.');
            yield this.runPlugins(this.options.plugins.afterInitialFetch);
            logger_1.logger.log('info', 'Running the `beforePreCssInlineRemoval` plugins.');
            yield this.runPlugins(this.options.plugins.beforePreCssInlineRemoval);
            // Running the pre CSS inline removals.
            logger_1.logger.log('info', 'Running pre CSS inline removal functions.');
            yield this.runPlugins(plugins.buildPreCssInlineRemovalsPluginPipeline(this));
            // Running the `afterPreCssInlineRemoval` and `beforeCssInlines` plugins.
            logger_1.logger.log('info', 'Running the `afterPreCssInlineRemoval` plugins.');
            yield this.runPlugins(this.options.plugins.afterPreCssInlineRemoval);
            logger_1.logger.log('info', 'Running the `beforeCssInlines` plugins.');
            yield this.runPlugins(this.options.plugins.beforeCssInlines);
            // Caching and importing all of the CSS code, completed inlined CSS is
            // needed for other inliners, such as the image inliners.
            logger_1.logger.log('info', 'Caching all CSS style sheets and imports.');
            this.cacheCss();
            logger_1.logger.log('info', 'Fetching and inlining CSS style sheets.');
            yield this.importAllCss();
            logger_1.logger.log('info', 'Inlining CSS style sheets.');
            yield this.inlineCss();
            // Running the `afterCssInlines` and `beforePostCssInlineRemoval` plugins.
            logger_1.logger.log('info', 'Running the `afterCssInlines` plugins.');
            yield this.runPlugins(this.options.plugins.afterCssInlines);
            logger_1.logger.log('info', 'Running the `beforePostCssInlineRemoval` plugins.');
            yield this.runPlugins(this.options.plugins.beforePostCssInlineRemoval);
            // Running the post CSS inline removal plugins.
            logger_1.logger.log('info', 'Running post CSS inline removal functions.');
            yield this.runPlugins(plugins.buildPostCssInlineRemovalsPluginPipeline(this));
            // Running the `afterPostCssInlineRemoval` plugins.
            logger_1.logger.log('info', 'Running the `afterPostCssInlineRemoval` plugins.');
            yield this.runPlugins(this.options.plugins.afterPostCssInlineRemoval);
            // Caching the remaining resources in the document.
            logger_1.logger.log('info', 'Caching all script resource URLs.');
            this.cacheScripts();
            logger_1.logger.log('info', 'Caching all HTML resource URLs.');
            this.cacheHtmlImages();
            logger_1.logger.log('info', 'Caching all CSS resource URLs.');
            this.cacheCssImages();
            logger_1.logger.log('info', 'Caching all audio resource URLs.');
            this.cacheAudio();
            logger_1.logger.log('info', 'Caching all video resource URLs.');
            this.cacheVideos();
            logger_1.logger.log('info', 'Caching all font resource URLs.');
            this.cacheFonts();
            logger_1.logger.log('info', 'Caching all cursor resource URLs.');
            this.cacheCursors();
            logger_1.logger.log('info', 'Caching all favicon resource URLs.');
            this.cacheFavicons();
            logger_1.logger.log('info', 'Caching other resource URLs.');
            this.cacheOtherResources();
            // Running the `beforePostCssInlineResourceFetch` plugins.
            logger_1.logger.log('info', 'Running the `beforePostCssInlineResourceFetch` plugins.');
            yield this.runPlugins(this.options.plugins.beforePostCssInlineResourceFetch);
            // Fetching all the cached resources
            logger_1.logger.log('info', 'Fetching all cache resources.');
            yield this.fetchAllCachedResources();
            // Running the `afterPostCssInlineResourceFetch` and
            // `beforeResourceModifications` plugins.
            logger_1.logger.log('info', 'Running the `afterPostCssInlineResourceFetch` plugins.');
            yield this.runPlugins(this.options.plugins.afterPostCssInlineResourceFetch);
            logger_1.logger.log('info', 'Running the `beforeResourceModifications` plugins.');
            yield this.runPlugins(this.options.plugins.beforeResourceModifications);
            // Running the resource modifications plugins.
            logger_1.logger.log('info', 'Running resource modification functions.');
            yield this.runPlugins(plugins.buildResourceModificationsPluginPipeline(this));
            // Running the `afterResourceModifications` and
            // `beforeResourceInlines` plugins.
            logger_1.logger.log('info', 'Running the `afterResourceModifications` plugins.');
            yield this.runPlugins(this.options.plugins.afterResourceModifications);
            logger_1.logger.log('info', 'Running the `beforeResourceInlines` plugins.');
            yield this.runPlugins(this.options.plugins.beforeResourceInlines);
            // Inlining the remaining resources
            logger_1.logger.log('info', 'Inlining scripts.');
            yield this.inlineScripts();
            logger_1.logger.log('info', 'Inlining HTML images.');
            yield this.inlineHtmlImages();
            logger_1.logger.log('info', 'Inlining audio resources.');
            yield this.inlineAudio();
            logger_1.logger.log('info', 'Inlining video resources.');
            yield this.inlineVideos();
            logger_1.logger.log('info', 'Inlining favicons resources.');
            yield this.inlineFavicons();
            logger_1.logger.log('info', 'Inlining CSS URL resources.');
            yield this.inlineCssResources();
            logger_1.logger.log('info', 'Inlining other resources.');
            yield this.inlineOtherResources();
            // Running the `afterResourceInlines` and `beforeFinalInlineModifications`
            // plugins.
            logger_1.logger.log('info', 'Running the `afterResourceInlines` plugins.');
            yield this.runPlugins(this.options.plugins.afterResourceInlines);
            logger_1.logger.log('info', 'Running the `beforeFinalInlineModifications` plugins.');
            yield this.runPlugins(this.options.plugins.beforeFinalInlineModifications);
            // Running the final inline modifications plugins.
            logger_1.logger.log('info', 'Running final inline modifications functions.');
            yield this.runPlugins(plugins.buildFinalInlineModificationsPluginPipeline(this));
            // Running the `afterFinalInlineModifications` and `beforeFinalHtmlCreation`
            // plugins.
            logger_1.logger.log('info', 'Running the `afterFinalInlineModifications` plugins.');
            yield this.runPlugins(this.options.plugins.afterFinalInlineModifications);
            logger_1.logger.log('info', 'Running the `beforeFinalHtmlCreation` plugins.');
            yield this.runPlugins(this.options.plugins.beforeFinalHtmlCreation);
            // Running the final document creation plugins.
            logger_1.logger.log('info', 'Running final inline modifications functions.');
            yield this.runPlugins(plugins.buildFinalDocumentCreationPluginPipeline(this));
            ...
            logger_1.logger.log('info', 'Running HTML page bundling functions.');
            yield this.runPlugins(plugins.buildFinalHtmlCreationPluginPipeline(this));
            // Recreating the DOCTYPE and adding it to the final HTML page bundle.
            const doctype = utilities.getDoctype(this.document);
            const outHtml = `${doctype}${this.html}`;
            // Running the `afterFinalHtmlCreation` plugins.
            logger_1.logger.log('info', 'Running the `afterFinalHtmlCreation` plugins.');
            yield this.runPlugins(this.options.plugins.afterFinalHtmlCreation);
            // If headless browser transport is specified, cleaning up the prior
            // browser instance and closing the browser.
            if (this.options.headlessBrowserTransport) {
                logger_1.logger.log('info', 'Closing up headless browser.');
                yield this.browser.close();
            }
            // If the `--out` flag is set, writing the output to a specified file.
            // Else, write the final HTML output to STDOUT.
            if (this.options.out) {
                logger_1.logger.log('info', `Bundle written to: ${this.options.out}.`);
                fs.writeFileSync(this.options.out, outHtml);
            }
            else {
                logger_1.logger.log('info', 'Writing bundle to STDOUT.');
                console.log(outHtml);
            }
        });
    }
}
exports.HyperFiler = HyperFiler;
