...
/*
test('HyperFiler.inlineCss() => All external style sheets linked are inlined.', t => {
  const tempDir: string = path.join(__dirname, 'inlineCss_test');
  ...
  for (const resourceDetail of resourceDetails) {
    const url: string = resourceDetail.url;
    const contents: string = resourceDetail.contents;
    const filePath: string = path.join(tempDir, url);

    fs.writeFileSync(filePath, contents);
  }

  const hyperFiler = new HyperFiler({ url: 'index.html' });
  const document: Document = hyperFiler.document;

  hyperFiler.cacheCss();
  hyperFiler.inlineCss();

  const shouldBeTrues: boolean[] = [
    [...document.getElementsByTagName('style')].length === 2,
  ];

  fs.rmdirSync(tempDir, { recursive: true });

  t.is(shouldBeTrues.every(value => value === true), true);
});
*/